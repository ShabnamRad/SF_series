...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun n => evenb n && negb (leb n 7)) l.
...
Proof.
  intros. induction l as [|x l' IHl'].
  - reflexivity.
  - simpl. assert (H: forall l1 l2, map f (l1 ++ l2) = map f l1 ++ map f l2).
...
(* Yes. e.g. checking if an element is present in a natlist: X: nat Y: bool*)
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y :=
  fold (fun elem res_list => (f elem) :: res_list) l nil.
...
Proof.
  intros. replace p with (fst p, snd p). reflexivity.
...
(*destruct n as [|n'].
  - intros H. apply length_nil in H. rewrite -> H. reflexivity.
