...
Proof.
  ...
  - simpl. assert(H: forall a b: nat, a * S b = a * b + a).
      {intros a b. induction a as [|a' IHa'].
        - reflexivity.
        - simpl. rewrite <- plus_n_Sm. rewrite -> IHa'. rewrite -> plus_assoc. reflexivity. }
      simpl. rewrite -> H. rewrite -> IHn'. rewrite -> plus_comm. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  replace (m + n) with (n + m). reflexivity. rewrite -> plus_comm. reflexivity.
Qed.
(** [] *)
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  |Z => O
  |A m' => double (bin_to_nat m')
  |B m' => S( double (bin_to_nat m'))
  end.
...
(*Fixpoint divide_by_two_nat (n: nat): nat :=
  match n with
  |O => O
  |S O => O
  |S (S n') => S (divide_by_two_nat n')
end.

Definition divide_by_two_bin (n: bin): bin :=
  match n with
  |Z => Z
  |B Z => Z
  |A n' => n'
...
(*|S m => match evenb (S m) with
          |true => A (divide_by_two_bin (incr (nat_to_bin m)))
          |flase => B (divide_by_two_bin (nat_to_bin m))
         end
  end.

Example test_nat_to_bin1: nat_to_bin( S( S( S( S( S O))))) = B (A (B Z)).
Proof. simpl. reflexivity. Qed.
Example test_nat_to_bin12: nat_to_bin( S( S( S( S( S( S( S( S O)))))))) = A (A (A (B Z))).
Proof. simpl. reflexivity. Qed.*)
...
Proof.
  intros n. induction n as [|n' IHn'].
  - reflexivity.
  - simpl. replace (S n') with (S (bin_to_nat (nat_to_bin n'))).
    rewrite -> bin_to_nat_pres_incr. reflexivity.
    rewrite -> IHn'. reflexivity.
Qed.
...
(* Extra As, i.e. 0s on the right, will be omitted. e.g. nat_to_bin (bin_to_nat (B (A (A Z)))) = B Z *)
...
Definition omit_extra_zeros (m: bin): bin :=
  match m with
  |Z => Z
  |x => A x
  end.
...
Example normalizertest1: normalize( B( A (A (B (A (A (A (A Z)))))))) = B (A (A (B Z))).
Proof. simpl. reflexivity. Qed.

Lemma double_zero: forall n, nat_to_bin n = Z -> nat_to_bin (double n) = Z.
Proof.
  intros. destruct n as [|n'].
  - reflexivity.
  - assert (L: forall n, nat_to_bin n = Z -> n = O).
    { intros n. destruct n. intros. reflexivity. simpl. destruct (nat_to_bin n).
      - intros contra. discriminate contra.
      - intros contra. discriminate contra.
      - intros contra. discriminate contra. }
    apply L in H. discriminate H.
Qed.

Lemma double_A_same_func: forall n b, nat_to_bin n = A b -> nat_to_bin (double n) = A (A b).
Proof.
  intros. induction n as [|n'].
  - discriminate H.
  - simpl. destruct (nat_to_bin (double n')).
    + discriminate IHn'. Admitted.


Theorem bin_to_nat_to_bin_normalizer: forall (m: bin), nat_to_bin (bin_to_nat m) = normalize  m.
Proof.
  intros. induction m as [|m'|m'].
  - reflexivity.
  - simpl. destruct (normalize m') eqn:E.
    + destruct m'.
      * reflexivity.
      * apply double_zero in IHm'. apply IHm'.
      * apply double_zero in IHm'. apply IHm'.
    + destruct m'.
      * reflexivity.
      * Admitted.
(*intros m. induction m as [|m' IHm'|m' IHm'].
  - reflexivity.
  - destruct (bin_to_nat (A m')).
    + simpl. destruct m'.
      * reflexivity.
      * 
    + *)

Lemma double_is_zero: forall (a: bin), bin_to_nat a =? 0 = (double (bin_to_nat a) =? 0).
  intros a. destruct a as [|a'|b'] eqn:LA.
  - reflexivity.
  - destruct (bin_to_nat (A a')). reflexivity. reflexivity.
  - destruct (bin_to_nat (B b')). reflexivity. reflexivity.
Qed.

Lemma norm_A_a: forall a: bin, bin_to_nat a =? 0 = false -> normalize(A a) = A (normalize a).
Proof.
  intros a H. induction a as [|a' IHa'|b' IHb'].
  - simpl in H. discriminate H.
  - simpl in H. rewrite <- double_is_zero in H. simpl. destruct a'.
    + simpl in H. discriminate H.
    + Admitted.

Lemma norm_invariant : forall m : bin, 
  normalize (normalize m) = normalize m.
Proof.
  intros m. destruct (normalize m) as [|a'|b'] eqn:Norm.
  - reflexivity.
  - simpl. destruct a'.
