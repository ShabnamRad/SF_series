...
Theorem silly_ex :
     ...
     oddb 3 = true ->
     evenb 4 = true.
Proof. intros eq1 eq2. apply eq2. Qed.
(** [] *)
...
Example injection_ex3 : forall (X : Type) (x y z : X) (l j : list X),
  ...
  x = y.
Proof.
  intros. injection H0 as H1 H2. rewrite H1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. destruct m as [|m'].
      ...
      + simpl in H. rewrite <- plus_n_Sm in H. rewrite <- plus_n_Sm in H.
        injection H as H. apply IHn' in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
Theorem length_nil: forall X l, @length X l = 0 -> l = nil.
Proof.
  destruct l as [|x l'] eqn:E.
  - reflexivity.
  - simpl. intros L. discriminate L.
Qed.

Theorem nth_theory: forall X n x l',
  (n =? 0) = false -> @nth_error X (x :: l') n = @nth_error X l' (pred n).
Proof.
...
Proof.
  ...
  - intros n eq. destruct n as [|n'].
    + apply length_nil in eq. rewrite -> eq. reflexivity.
    + rewrite -> nth_theory.
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
   intros X Y l. induction l as [|p l' IHl'].
  - intros. injection H. intros H1 H2. rewrite <- H1. rewrite <- H2. reflexivity.
  - intros. simpl in H. destruct p as [x y]. destruct (split l') as [l1' l2'].
    injection H. intros H1 H2. rewrite <- H1. rewrite <- H2. simpl.
    assert(L: forall (X: Type) (h: X) (t1 t2: list X), t1 = t2 -> h :: t1 = h :: t2).
    { intros. rewrite H0. reflexivity. } apply L. apply IHl'. reflexivity.
Qed.
(** [] *)
...
Theorem eqb_sym : forall (n m : nat),
  (n =? m) = (m =? n).
Proof.
  intros. destruct (n =? m) eqn:Enm.
  - apply eqb_true in Enm. rewrite Enm. apply eqb_refl.
  - destruct (m =? n) eqn: Emn.
    + apply eqb_true in Emn. rewrite Emn in Enm. rewrite <- eqb_refl in Enm. discriminate Enm.
    + reflexivity.
Qed.
(** [] *)
...
Theorem eqb_trans : forall n m p,
  ...
  n =? p = true.
Proof.
  intros. apply eqb_true in H. apply eqb_true in H0. rewrite H0 in H.
  destruct (n =? p) eqn:E.
  - reflexivity.
  - rewrite H in E. rewrite <- eqb_refl in E. discriminate E.
Qed.
(** [] *)
...
Theorem tail_length: forall (X Y: Type) (x: X) (l1: list X) (y: Y) (l2: list Y),
  length (x :: l1) = length (y :: l2) -> length l1 = length l2.
Proof.
  intros. destruct l1 as [|x' l1'].
  - simpl in H. injection H as H. symmetry in H. apply length_nil in H. rewrite H. reflexivity.
  - simpl in H. injection H as H. simpl. apply H.
Qed.

Theorem split_combine : split_combine_statement.
Proof.
  unfold split_combine_statement. intros X Y l1. induction l1 as [|x l1' IHl'].
  - intros. simpl. simpl in H. symmetry in H. apply length_nil in H. rewrite H. reflexivity.
  - intros. destruct l2 as [|y l2'].
    + discriminate H.
    + unfold split. simpl. rewrite IHl'. reflexivity. apply tail_length in H. apply H.
Qed.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
     ...
     test x = true.
Proof.
  intros. generalize dependent x. generalize dependent lf. induction l as [|x' l'].
  - intros. discriminate H.
  - intros. simpl in H. destruct (test x') eqn:Testx'.
    + injection H as H1 H2. rewrite H1 in Testx'. apply Testx'.
...
Fixpoint forallb {X : Type} (test : X -> bool) (l : list X) : bool :=
  fold (fun x res => res && test x) l true.
...
Fixpoint existsb {X : Type} (test : X -> bool) (l : list X) : bool :=
  fold (fun x res => res || test x) l false.
...
Proof.
  ...
  - reflexivity.
  - simpl. unfold existsb'. simpl. destruct (test x') eqn:Test.
    + assert(H1: forall b, b || true = true). { intros. destruct b. reflexivity. reflexivity. } rewrite H1.
    assert (H2: forall b, b && false = false). { intros. destruct b. reflexivity. reflexivity. } rewrite H2.
    reflexivity.
    + assert(H1: forall b, b || false = b). { intros. destruct b. reflexivity. reflexivity. } rewrite H1.
    assert (H2: forall b, b && true = b). { intros. destruct b. reflexivity. reflexivity. }  rewrite H2.
    unfold existsb' in IHl'.
    replace (forallb (fun x : X => negb (test x)) l') with (fold (fun (x : X) (res : bool) => res && negb (test x)) l' true) in IHl'.
    rewrite <- IHl'. Admitted.
