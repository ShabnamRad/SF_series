...
Proof.
  intros [|n] m H.
  - left. reflexivity.
  - simpl in H. apply and_exercise in H. apply proj1 in H. right. apply H.
Qed.
(** [] *)
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H Q contra. unfold not in H. apply H in contra. destruct contra. Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H HNQ. unfold not. intros HP. unfold not in HNQ.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros. unfold not. intros [HP PF]. apply PF. apply HP.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [[HP1 | HQ] [HP2 | HR]]. left. apply HP1. left. apply HP1.
      left. apply HP2. right. split. apply HQ. apply HR.
Qed.
(** [] *)
...
Proof.
  intros. unfold not. intros E. destruct E as [x Hx]. apply Hx. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [[x H] | [x H]]. exists x. left. apply H. exists x. right. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction l as [|x l' IHl'].
    ...
    + simpl in H. destruct H.
      ...
      * apply IHl' in H. destruct H as [x' [HF HIn]]. exists x'. split.
        apply HF. simpl. right. apply HIn.
  - intros H. destruct H as [x [HF HIn]]. induction l as [|x' l' IHl'].
    + destruct HIn.
    + simpl in HIn. destruct HIn.
      * simpl. left. rewrite <- H in HF. apply HF.
...
Proof.
  intros. split.
  - induction l as [|x lx IHl].
    + simpl. intros. right. apply H.
    + simpl. intros [H | H].
      * left. left. apply H.
      * apply IHl in H. rewrite <- or_assoc. right. apply H.
  - induction l as [|x lx IHl].
    + simpl. intros [F | H]. destruct F. apply H.
    + simpl. intros H. rewrite <- or_assoc in H. destruct H.
      * left. apply H.
...
Proof.
  ...
  - intros. induction l as [|x' l' IHl].
    ...
    + simpl. split.
      ...
      * simpl in H. apply IHl. intros x Hx. apply H. right. apply Hx.
  - induction l as [|x' l' IHl].
    + intros. destruct H0.
    + simpl. intros [HP HA]. intros x [H | H].
      * rewrite H in HP. apply HP.
      * apply IHl in H. apply H. apply HA.
Qed.
(** [] *)
...
Proof.
  intros. unfold combine_odd_even. destruct (oddb n). apply H. reflexivity. apply H0. reflexivity.
Qed.
...
Proof.
  ...
  unfold tr_rev. induction x as [|x' l' IHx].
  - reflexivity.
  - simpl. rewrite <- IHx.
  assert ( H: forall T l1 l2, @rev_append T l1 l2 = @rev_append T l1 [] ++ l2).
  { intros T. induction l1 as [|h t IHl].
    - reflexivity.
    - simpl. rewrite IHl. intros.
      rewrite IHl with (l2 := h :: l2).
