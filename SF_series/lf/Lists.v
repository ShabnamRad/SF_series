...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  |nil => true
  |h :: t => andb (andb (member h s2) (leb (count h s1) (count h s2))) (subset t s2)
  end.
...
Theorem bag_theorem : forall v b, count v (add v b) =? 0 = false.
Proof.
  intros v b. assert(H: count v (add v b) = S (count v b)).
  { simpl. replace (v =? v) with true. reflexivity. rewrite <- eqb_refl. reflexivity. }
...
Proof.
  intros. rewrite -> app_assoc, app_assoc. reflexivity.
Qed.
...
Proof.
  intros. induction l1 as [|n l' IHl'].
  - reflexivity.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  |nil, _ => false
  |_, nil => false
  |n1 :: l1', n2 :: l2' => andb (n1 =? n2) (eqblist l1' l2')
  end.
...
Proof.
  intros. induction s1 as [|n s' IHs'].
  - reflexivity.
  - simpl. destruct (v =? n) as [].
